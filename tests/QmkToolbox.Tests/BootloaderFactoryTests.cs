...
public class BootloaderFactoryTests
{
        ...
        ushort vid, ushort pid, ushort rev, BootloaderType expected)
    {
        ...
        BootloaderDevice? bd = BootloaderFactory.CreateDevice(device, new BootloaderServices(toolProvider, new FakeUsbResolver(), (_, _) => { }));
    ...
    {
        // VID/PID are arbitrary: UF2 mass-storage devices match on marker file, not ID.
        UsbDeviceInfo device = Usb(0x239A, 0x00FF);
        IFlashToolProvider toolProvider = Substitute.For<IFlashToolProvider>();

        BootloaderDevice bd = BootloaderFactory.CreateMassStorageDevice(BootloaderType.Uf2, device, new BootloaderServices(toolProvider, new FakeUsbResolver(), (_, _) => { }), boardId: boardId);
    ...
    public void CreateMassStorageDevice_NonMassStorageType_Throws()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(
            () => BootloaderFactory.CreateMassStorageDevice(BootloaderType.AtmelDfu, device, new BootloaderServices(toolProvider, new FakeUsbResolver(), (_, _) => { })));
    }
    ...
    public void CreateDevice_UnknownVidPid_ReturnsNull()
    {
        ...
        BootloaderDevice? result = BootloaderFactory.CreateDevice(device, new BootloaderServices(toolProvider, new FakeUsbResolver(), (_, _) => { }));
