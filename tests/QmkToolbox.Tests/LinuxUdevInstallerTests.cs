...
{
    /// <summary>Splits the installer's single sink back into separate stdout and stderr lists.</summary>
    private static MessageSink Sink(List<string> output, List<string> errors) =>
        (text, type) => (type == MessageType.Error ? errors : output).Add(text);
    ...
    [FactOnLinux]
    public async Task InstallAsync_RunsGeneratedScriptUnderPkexec_AndForwardsOutputAsync()
    {
        ...
        try
        {
            await LinuxUdevInstaller.InstallAsync(provider, Sink(output, errors), runner);
    ...
    [FactOnLinux]
    public async Task InstallAsync_NonZeroExit_ReportsErrorAsync()
    {
        (IFlashToolProvider provider, string folder) = ResourceFolderWithUdevFiles();
        ...
        {
            var errors = new List<string>();
            await LinuxUdevInstaller.InstallAsync(provider, Sink([], errors), new ScriptCapturingRunner(exitCode: 127));
    ...
    [FactOnLinux]
    public async Task InstallAsync_MissingResources_ReportsErrorWithoutLaunchingAsync()
    {
        IFlashToolProvider provider = Substitute.For<IFlashToolProvider>();
        ...
        await LinuxUdevInstaller.InstallAsync(provider, Sink([], errors), runner);
