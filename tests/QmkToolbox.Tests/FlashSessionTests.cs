...
public sealed class FlashSessionTests : IDisposable
{
    ...
    private sealed class Harness : IDisposable
    {
        ...
        public Harness()
        {
            ...
            // Mirrors the composition root: one output for the devices, the orchestrator, and
            // the session.
            void output(string msg, MessageType type)
            { lock (_output) { _output.Add((msg, type)); } }
            Orchestrator = new FlashOrchestrator(new BootloaderServices(ToolProvider, resolver, output)
            {
                ProcessRunner = new CapturingProcessRunner(),
            });
            Session = new FlashSession(f => f(), Detector, Orchestrator, ToolProvider, output);
        }
    ...
    public void UnknownDevice_ShowAllDisabled_Silent()
    {
        ...
        Assert.False(_h.Session.CanFlash);
        // Trace shares the sink, so silence means nothing the user sees, not nothing at all.
        Assert.DoesNotContain(_h.Snapshot(), o => o.Type != MessageType.Debug);
    }
