...
public sealed class MainWindowViewModelTests : IDisposable
{
    ...
    public MainWindowViewModelTests()
    {
        ...
        var orchestrator = new FlashOrchestrator(new BootloaderServices(_toolProvider, new FakeUsbResolver(), (_, _) => { })
        {
    ...
    [Fact]
    public async Task OpenFile_PickedFile_SelectsFirmwareAsync()
    {
    ...
    [Fact]
    public async Task OpenFile_PickerCancelled_KeepsCurrentFirmwareAsync()
    {
    ...
    public void DeviceConnected_EnablesFlashCommand()
    {
        MainWindowViewModel vm = NewVm();
        bool raised = false;
        vm.Session.FlashCommand.CanExecuteChanged += (_, _) => raised = true;
        Assert.False(vm.Session.FlashCommand.CanExecute(null));

        _detector.RaiseConnected(new UsbDeviceInfo(0x03EB, 0x2FEF, 0, "", "", ""));

        Assert.True(raised);
        Assert.True(vm.Session.FlashCommand.CanExecute(null));
    }

    // ── first-start setup ─────────────────────────────────────────────────────

    [Fact]
    public async Task FirstStart_Linux_PromptsForUdevRules_DeclineClearsFlagAsync()
    {
    ...
    [Fact]
    public async Task FirstStart_Windows_PromptsForDriverInstall_DeclineClearsFlagAsync()
    {
        _settings.Current.FirstStart = true;
    ...
    [Fact]
    public async Task FirstStart_MacOS_NoPrompt_StillClearsFlagAsync()
    {
        _settings.Current.FirstStart = true;
        MainWindowViewModel vm = NewVm();
