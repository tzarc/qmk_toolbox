...
using static Interop.Libc;

namespace QmkToolbox.Usb.Discovery.Linux;

/// <summary>
/// Linux probe: hotplug via a raw netlink kobject-uevent socket (no udevd, no libudev), sysfs
/// enrichment, and a /sys/bus/usb sweep of already-present devices. Kernel uevents deliver
/// exactly one add and one remove per USB device (<c>DEVTYPE=usb_device</c>), both carrying
/// VID/PID/bcdDevice in <c>PRODUCT=</c>.
/// </summary>
internal sealed class LinuxUsbProbe : IUsbProbe
{
    ...
    /// (interface and endpoint entries have neither), and hubs (bDeviceClass 09) are skipped.
    /// Symlinked entries resolve to the canonical /sys/devices/… syspath so swept devices dedup
    /// against later uevents for the same device.
    /// </summary>
