...
public sealed class UsbDeviceTracker(IUsbProbe probe) : IUsbEventsDetector
{
    ...
    public void Start()
    {
        ...
        probe.Start();
        // The sweep runs after subscription so no arrival slips between the two; OnArrived's
        // duplicate-path guard drops a device delivered by both.
        ...
        lock (_devicesLock)
        {
                ...
                _devices.Any(d => string.Equals(d.DevicePath, device.DevicePath, probe.PathComparison)))
            {
                DiagnosticTrace?.Invoke(
                    $"[USB+] duplicate arrival ignored {device.TraceVidPidRev} path:{device.TracePath}");
                return;
            }
            _devices.Add(device);
        }
        DiagnosticTrace?.Invoke(
            $"[USB+] {device.TraceVidPidRev} path:{device.TracePath}");
    ...
    private void OnRemoved(UsbRemovalHint hint)
    {
        ...
        {
            DiagnosticTrace(
                $"[USB-] event path:{hint.TracePath} VID:{hint.VendorId:X4} PID:{hint.ProductId:X4}");
            if (existing != null)
            {
                DiagnosticTrace(matchedByPath
                    ? $"[USB-] matched by path  ({existing.TraceVidPid})"
                    : $"[USB-] matched by VID/PID ({vidPidCandidates} candidate(s))  ({existing.TraceVidPid})");
            }
            else
            {
