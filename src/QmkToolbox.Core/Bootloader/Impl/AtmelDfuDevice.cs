...
{
    public AtmelDfuDevice(UsbDeviceInfo device, BootloaderServices services, BootloaderType type)
    ...
    public override async Task FlashEepromAsync(string mcu, string file)
    {
        ...
        if (Type == BootloaderType.AtmelDfu)
            Output("Please reflash device with firmware now", MessageType.Bootloader);
    }
