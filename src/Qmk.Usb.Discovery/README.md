...
USB device hotplug detection for .NET on Windows, Linux, and macOS: subscribe to connect and disconnect events, pick up the devices already attached at startup, and check which USB device backs a mounted volume.
...
.NET 10 on Windows, Linux, or macOS 13+. The package covers detection, identity, and volume topology; device I/O is out of scope.
