...
/// commands reference the ViewModel and its session directly, and checkable items track their
/// source property through its change notifications. The macOS application menu (the bold "QMK Toolbox"
...
public static class AppMenu
{
    ...
    {
        bool isMacOS = !vm.IsWindows && !vm.IsLinux;
        // Meta is Command on macOS and Super elsewhere, but Window.KeyBindings binds Ctrl, so
        // every other platform must advertise Ctrl or the menu shows a shortcut that does nothing.
        KeyModifiers accelerator = isMacOS ? KeyModifiers.Meta : KeyModifiers.Control;
        ...
        var fileMenu = new NativeMenu
        {
            new NativeMenuItem("Open...")
            {
                ...
                Gesture = new KeyGesture(Key.O, accelerator)
            }
        };
        // macOS carries Quit in the application menu; every other platform needs it here.
        if (!isMacOS)
        {
            fileMenu.Add(new NativeMenuItemSeparator());
            fileMenu.Add(new NativeMenuItem("Exit") { Command = vm.ExitCommand });
        }

        var eepromMenu = new NativeMenu
        {
            new NativeMenuItem("Clear EEPROM") { Command = vm.Session.ClearEepromCommand },
            new NativeMenuItem("Set Left Hand") { Command = vm.Session.SetLeftHandCommand },
            new NativeMenuItem("Set Right Hand") { Command = vm.Session.SetRightHandCommand }
        };
        ...
        {
            new NativeMenuItem("Flash") { Command = vm.Session.FlashCommand },
            new NativeMenuItem("Exit DFU") { Command = vm.Session.ResetCommand },
        ...
        if (vm.IsWindows)
            toolsMenu.Add(new NativeMenuItem("Install Drivers...") { Command = vm.InstallDriversCommand });
        if (vm.IsLinux)
            toolsMenu.Add(new NativeMenuItem("Install udev Rules") { Command = vm.InstallUdevRulesCommand });
        toolsMenu.Add(new NativeMenuItem("Clear Resources") { Command = vm.Session.ClearResourcesCommand });
        toolsMenu.Add(new NativeMenuItemSeparator());
        toolsMenu.Add(new NativeMenuItem("Theme") { Menu = themeMenu });
