...
public partial class LogPanel : UserControl
{
    ...
    public LogPanel()
    {
        ...
        // One flyout on both the text block and the scroll viewer. On the text block it replaces
        // SelectableTextBlock's built-in Copy-only flyout, which otherwise swallows every
        // right-click over the full-width, hit-testable text area; on the scroll viewer it
    ...
    // The flyout's popup has its own name scope, so {Binding #Root...} on resource-declared
    // items never resolves. The items reference the commands directly, binding CopyCommand and
    // ClearCommand through their property observables because XAML sets them after construction.
