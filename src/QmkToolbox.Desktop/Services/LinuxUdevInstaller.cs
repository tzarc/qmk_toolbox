...
public static class LinuxUdevInstaller
{
    ...
    public static async Task InstallAsync(
        IFlashToolProvider toolProvider, MessageSink output,
        IProcessRunner? processRunner = null)
    {
        ...
        {
            output("udev resources not found. Please clear and re-extract resources via Tools → Clear Resources.", MessageType.Error);
            return;
        }
        ...
        try
        {
            ...
            try
            {
                ...
                Task stdoutTask = ReadLinesAsync(process.StandardOutput, line => output(line, MessageType.UdevOutput));
                Task stderrTask = ReadLinesAsync(process.StandardError, line => output(line, MessageType.Error));

                await Task.WhenAll(stdoutTask, stderrTask);
                await process.WaitForExitAsync(CancellationToken.None);

                if (process.ExitCode != 0)
                    output($"udev installation failed with exit code {process.ExitCode}.", MessageType.Error);
            }
            catch (System.ComponentModel.Win32Exception)
            {
                output("Could not start pkexec.", MessageType.Error);
            }
            catch (Exception ex)
            {
                output($"udev installation failed: {ex.Message}", MessageType.Error);
            }
        }
        finally
        {
