...
public class FlashToolProvider(string? resourceFolder = null, Assembly? resourceAssembly = null) : IFlashToolProvider
{
    ...
    /// <summary>
    /// Creates the resource folder if it is absent and extracts any bundled resource that is
    /// missing. When an installed manifest's COMMIT_DATE differs from its embedded copy, wipes
    /// the folder first so everything is re-extracted. Extraction skips files that already
    /// exist, so the common path is cheap.
    /// </summary>
    ...
    /// <summary>
    /// Deletes the resource folder and re-extracts every bundled resource.
    /// </summary>
