...
#         artifacts/qmk_toolbox-macOS.app.zip
...
rm -f "${ARTIFACTS_DIR}/qmk_toolbox-macOS.app.zip"
docker run --rm -i \
    -v "${REPO_ROOT}:/app" \
    -w /app \
    -e TC_WORKDIR=/app \
    ...
    bash << 'ZIP'
set -euo pipefail
cd publish-osx-dmg && zip -r "../artifacts/qmk_toolbox-macOS.app.zip" "QMK Toolbox.app"
ZIP

echo "  Done: ${ARTIFACTS_DIR}/qmk_toolbox-macOS.app.zip"
