...
#
# Packages whatever bundle is on disk, so a DMG built after the signing step
# picks up the signed bundle.
#
# Input:  publish-osx-dmg/QMK Toolbox.app  (from make-macos-app.sh)
# Output: artifacts/qmk_toolbox-macOS.dmg
...
rm -f "${ARTIFACTS_DIR}/qmk_toolbox-macOS.dmg"
docker run --rm -i \
    -v "${REPO_ROOT}:/app" \
    -w /app \
    -e TC_WORKDIR=/app \
    ...
    bash << 'DMG'
set -euo pipefail
genisoimage -V "QMK Toolbox" -D -R -apple -no-pad -o "artifacts/qmk_toolbox-macOS.dmg" publish-osx-dmg
DMG

echo "  Done: ${ARTIFACTS_DIR}/qmk_toolbox-macOS.dmg"
