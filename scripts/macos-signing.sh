#!/usr/bin/env bash
# macos-signing.sh: Code-sign and notarize QMK Toolbox using rcodesign.
#
...
# The bundle path contains a space ("QMK Toolbox.app"), which the containerised
# rcodesign invocation mishandles. Run the operation against a space-free
# sibling name, restoring the original name afterwards (even on failure). The
# (container) target path is appended as the final argument.
...
cmd_notarize() {
	local target="${1:?usage: notarize <path/to/App.app>}"
	require_setup
	[[ -e "$target" ]] || die "not found: $target"

	# --staple implies --wait; rcodesign zips .app bundles automatically for
	# upload and staples the ticket back onto the bundle in place.
...
main() {
	local command="${1:-}"
	[[ -n "$command" ]] || die "usage: $0 <setup|sign-resources|sign-app|notarize|cleanup> [args...]"
	shift || true
