#!/usr/bin/env bash
# fetch-tools.sh: Download release binaries from three upstream repositories and
# place them in the repository's resources/ tree:
#
#   qmk_flashutils        flash tool binaries (avrdude, dfu-util, etc.) for all platforms.
#                         The same release also carries the qmk_hidapi-* archives holding
#                         the hidapi native library; there is no qmk_hidapi repository.
